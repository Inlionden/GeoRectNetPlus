...
def load_single_image(path, size=512):
...
def main():
    parser = argparse.ArgumentParser(description="GeoRectNetPlus single-image inference")
    parser.add_argument("--image", required=True, help="Input satellite image")
    parser.add_argument("--checkpoint", default=os.environ.get("GEORECT_SAVE_PATH", cfg.save_path))
    parser.add_argument("--output", default="results/inference_mask.png")
    ...
    x, original_size = load_single_image(args.image, cfg.img_size)
    x = x.to(device)
    ...
    print(f"Saved building-footprint mask: {args.output}")
